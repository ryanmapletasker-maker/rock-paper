    ...
    <div class="card p-2 shadow-sm mb-4 p-3 bg-light text-center text-lg-left">
      <div class="row">
        ...
        <div class="col-lg-4 text-center">
          <?php if(!$userdata->ID){ ?>
          <a href="javascript:void(0)" <?php if(isset($_GET['action']) && $_GET['action'] == "register"){ ?>onclick="processRegister(); jQuery('.upgrade-modal-wrap').fadeOut(400);"<?php }else{ ?>onclick="processLogin(1);jQuery('.upgrade-modal-wrap').fadeOut(400);"<?php } ?> class="upgradebtn btn btn-primary mt-4 mt-lg-0" ><?php echo __("Select Package","premiumpress"); ?> </a>
          <?php }else{ ?>
          ...
          <div class="mt-2"><a href="javascript:void(0);" class="small" onclick="jQuery('#featuresfor<?php echo $n['key']; ?>').toggle();"><?php echo __("view features","premiumpress"); ?></a></div>
