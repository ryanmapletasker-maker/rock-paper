...
<div id="videoplayer">   <div class="addeditmenu" data-key="video"></div>
...
<?php if(isset($_POST['pid']) && !isset($GLOBALS['top-video']) ){  ?><hr /><?php } // mute="yes" poster="'.$videos[0]['thumbnail'].'" ?> 
 
 
<?php  echo do_shortcode('[video src="'.$videos[0]['src'].'" width="'.$width.'" height="'.$height.'" autoplay="on" ][/video]'); ?>
...
<?php } ?>
    ...
    <?php if(THEME_KEY == "vt" ){  // && !($userdata->ID) ?>
    ...
    <div class="container h-100 border px-md-5 bg-secondary text-white" id="upgrademessage" style="display:none;">
      <div class="row align-items-center h-100 my-md-5 py-5">
        <div class="col-8 mx-auto text-center my-lg-3">
          ...
          <a href="<?php echo _ppt(array('links','memberships')); ?>" class="btn btn-primary btn-lg mt-4"><?php echo __("Upgrade Now","premiumpress") ?></a> </div>
      </div>
    </div>
